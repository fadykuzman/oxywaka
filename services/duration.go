...
const (
	heartbeatPadding          = 0 * time.Second
	generateDurationsInterval = 12 * time.Hour
)
...
func (srv *DurationService) getLive(from, to time.Time, user *models.User, interval time.Duration, includeEntities bool) (models.Durations, error) {
	...
	// Aggregation
	// The below logic is approximately (no filtering, no "same day"-check).
	// A Postgres-compatible script was contributed by @cwilby and is available at scripts/aggregate_durations_postgres.sql
	// adding a distinction here to use pure-sql aggregation for Postgres, and traditional, programmatic aggregation for all other databases.
	var count int
